...
INSERT INTO Position    (PositionName)
 ...
 VALUE					('account1@gmail.com' ,	'Vinhvu'    ,		'Dũ Đức Vino'		,	'2'					,	'1'			,	'2020-03-05'),
						...
						('account6@gmail.com' , 'Minhle'    ,		'Nguyễn Đức Minh'  	,	'3'					,	'4'			,	'2020-03-05'),
...
DROP VIEW IF EXISTS list_of_staff_in_sales_department;
CREATE VIEW list_of_staff_in_sales_department as
SELECT A.FullName, A.Email, B.DepartmentName
FROM `Account` as A
...
WHERE B.DepartmentName = 'Sale';
 ...
 SELECT * from list_of_staff_in_sales_department;
...
DROP VIEW IF EXISTS account_group;
CREATE VIEW account_group AS

SELECT A.AccountID, A.Email, A.Username, A.FullName , count(B.AccountID) as SL
FROM `Account` as A
JOIN GroupAccount as B
ON A.AccountID = B.AccountID
GROUP BY B.AccountID 
HAVING count(B.AccountID) = ( SELECT max(countacc) from 
(select count(B.AccountID) as countacc from GroupAccount as B GROUP BY B.AccountID ) as TB_count
)
;
...
SELECT A.DepartmentName, group_concat(B.FullName) as thanh_vien, count(B.AccountID) as SL
FROM Department A
JOIN `Account` B
On A.DepartmentID = B.DepartmentID
GROUP BY A.DepartmentID 
HAVING count(B.DepartmentID) = 
(SELECT MAX(countdep) as maxcount from 
(select count(B.DepartmentID) as countdep from  `Account` B
GROUP BY B.DepartmentID) as TB_count
)
;
SELECT * from MaxNV;
...
DROP VIEW IF EXISTS tất_các_các_câu_hỏi_do_user_họ_Nguyễn_tạo;
CREATE VIEW tất_các_các_câu_hỏi_do_user_họ_Nguyễn_tạo as
SELECT A.Content, A.QuestionID, B.FullName
FROM   Question as A
JOIN   `Account` as B
ON 		A.CreatorID = B.AccountID
WHERE    B.FullName LIKE 'Nguyễn%';
SELECT * from tất_các_các_câu_hỏi_do_user_họ_Nguyễn_tạo;
